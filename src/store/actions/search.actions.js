...
export const SET_CLAN_RESULTS       = '[SEARCH] SET_CLAN_RESULTS';

export const setClanResults = (results) => ({
    type : SET_CLAN_RESULTS,
    results,
});
...
export const fetchClanResults = () => (dispatch) => (new Promise((resolve, reject) => {
    Api.doPost(endpoints.clan.buscar, {})
    .then(response => {
        const {error, error_controlado} = response;
        if(error || error_controlado) {
            reject(response);
            ...
            resolve(true);
            dispatch(setClanResults(response));
        }
        //dispatch(stopLoading());        
    })
    .catch(response => {
